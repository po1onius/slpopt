...
use md5::{Digest, Md5};
use reqwest::{self, header::{HeaderValue, CONTENT_TYPE, USER_AGENT}};
use rand::Rng;
...
impl TransRequest {
    ...
    async fn baidu(&self, text: &str) -> String {
        ...
        let h = format!("{}{}{}{}", baidu.appid.as_str(), text, salt, baidu.token.as_str());
        ...
        let body = [("q", text), ("from", "auto"), ("to", self.config.language.as_str()), ("appid", baidu.appid.as_str()), ("salt", salt.as_str()), ("sign", sign.as_str())];

        let response = self.req_client.post(BAIDU_URL).form(&body).header(CONTENT_TYPE, HeaderValue::from_static("application/x-www-form-urlencoded")).send().await.unwrap();
